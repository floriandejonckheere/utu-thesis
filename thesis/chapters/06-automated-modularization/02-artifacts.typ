...
The categories are based on #slr_cite_full(<bajaj_etal_2021>).
...
#slr_cite_full(<amiri_2018>) and #slr_cite_full(<daoud_etal_2020>) model a software system as a set of business process using the industry standard #acr("BPMN"), using the machine-readable XML representation as input for the algorithm.
#slr_cite_full(<yang_etal_2022>) tackle requirements engineering using problem frames @jackson_2000.
...
Some approaches use schematic requirements documents in XML format as input for the algorithm, as described by #slr_cite_full(<saidi_etal_2023>).
...
#slr_cite_full(<li_etal_2023>) employ an intermediate format containing a precise definition of business functionality, generated from validated requirements documents.
...
For example, #slr_cite_full(<al_debagy_martinek_2020>) propose a data-driven method based on the analysis of the software system's external API, specified in the OpenAPI#footnote[#link("https://www.openapis.org/")[https://www.openapis.org/]] format.
...
#slr_cite_full(<zhou_xiong_2022>) use readily available design documents as well, in the form of UML class diagrams, use cases, and object sequence diagrams as starting point for the microservice identification algorithm.
The decomposition tool proposed by #slr_cite_full(<hasan_etal_2023>) uses design documents as well, although the specifications are inferred from the source code of the software system, and do not require pre-existing design documents.

#slr_cite_full(<quattrocchi_etal_2024>) takes a different approach to the problem, using a data-driven approach combined with a domain-driven approach.
...
#slr_cite_full(<wei_etal_2020>) uses a similar approach, gathering a list of features from the software architect, and proposing a microservice decomposition based on pre-trained feature tables.
...
#slr_cite_full(<mazlami_etal_2017>) originally proposed the use of the revision history of the source code to identify couplings between classes.
...
#slr_cite_full(<escobar_etal_2016>) use the source code of the software system to construct an #acr("AST"), and map the dependencies between the business and data layer.
#slr_cite_full(<kamimura_etal_2018>) use a more data-driven approach, and statically trace data access calls in the source code.
...
#slr_cite_full(<bandara_perera_2020>) map object-oriented classes in the source code to specific microservices, but require a list of microservices to be specified before the decomposition is performed.

#slr_cite_full(<filippone_etal_2021>) concentrate on the API controllers as entrypoints into the software system.
...
Likewise, #slr_cite_full(<zaragoza_etal_2022>) make a distinction between presentation, business, and data layer.

Most of the publications tracing dependencies between classes (or modules) do this at the level of the classes (or modules).
As #slr_cite_full(<mazlami_etal_2017>) remark, using a more granular approach at the level of methods (or functions) and attributes has the potential to improve the quality of the decomposition.
#slr_cite_full(<carvalho_etal_2020>) use a more granular approach, identifying dependencies between methods in the source code.
On the other hand, #slr_cite_full(<kinoshita_kanuka_2022>) do not automatically extract information from the source code, but rely on a software architect to decompose the software system on the basis of business capability.

#slr_cite_full(<romani_etal_2022>) propose a data-centric microservice candidate identification method based on knowledge gathered from the database schema.
...
#slr_cite_full(<hao_etal_2023>) construct access patterns from both the database schema (static) and the database calls during execution of the software system (dynamic).

A unique approach to constructing a call graph is proposed by #slr_cite_full(<nitin_etal_2022>), who make a distinction between context-insensitive and context-sensitive dependency graphs.
...
Examples of approaches using execution traces are #slr_cite_full(<jin_etal_2021>) and #slr_cite_full(<eyitemi_reiff_marganiec_2020>).
...
#slr_cite_full(<zhang_etal_2020>) collect the execution traces of the software system, in combination with performance logs.

#slr_cite_full(<ma_etal_2022>) use a data-centric approach based on the analysis of database access requests.

==== Hybrid approach

Some publications suggest a hybrid approach using both static and dynamic analysis.
For instance, #slr_cite_full(<wu_zhang_2022>), #slr_cite_full(<carvalho_etal_2020>) and #slr_cite_full(<cao_zhang_2022>) collect information statically from the source code (entity classes and databases), as well as dynamically from the execution of the software system (execution traces).
The approach proposed by #slr_cite_full(<lourenco_silva_2023>) uses either static of the source code or dynamic analysis of the system execution to gather access patterns.

#slr_cite_full(<hao_etal_2023>) use both static and dynamic analysis, albeit aimed at the database schema and database calls, respectively.
