...
#cite_full(<al_debagy_martinek_2020>) use the inverse of cohesion as a metric, named lack of cohesion (LCOM).
...
#cite_full(<filippone_etal_2021>) and others @carvalho_etal_2020 @zhou_xiong_2022 calculate the value based using a heuristic function that uses the size of primitive types of method call arguments to predict the total network overhead of a microservice decomposition.
#cite_full(<carvalho_etal_2020>) also includes the protocol overhead in the calculation, which is the cost of the communication protocol used to send messages between services (for example, TCP headers, HTTP headers, etc.).

#cite_full(<quattrocchi_etal_2024>) measure network overhead as part of their operational cost metric.
...
#cite_full(<al_debagy_martinek_2020>) defines complexity based on Number of Operations, a metric that uses Weighted Methods per Class (WMC), summing the number of methods in a class.

#cite_full(<santos_silva_2022>) define the complexity metric in terms of the functionality redesign effort, rather than the complexity of the microservice candidates.
...
In another publication by the same co-author, #cite_full(<lourenco_silva_2023>) define complexity as the effort required to perform the decomposition, and expand the concept to uniform complexity, which is calculated by dividing the complexity of a decomposition by the maximum possible complexity.
...
#cite_full(<zhang_etal_2020>) use this metric to evaluate the quality of the microservice decomposition, by predicting the average CPU and memory usage of the microservices.
The prediction is made based on performance logs collected by executing the monolith application.

#cite_full(<quattrocchi_etal_2024>) define operational costs as metric to minimize, which includes communication (network) and data management (CPU and memory) costs.

#cite_full(<nitin_etal_2022>) do not utilize the CPU and memory usage directly as a metric, but instead assume the latency and throughput as indicators of performance.
...
#cite_full(<jin_etal_2021>) use modularity as a metric to evaluate potential decompositions.
...
#cite_full(<carvalho_etal_2020>) introduce a metric named feature modularization, which maps a list of features supplied by the software architect onto classes and methods, determining the set of predominant features per microservice.

==== Other metrics

#cite_full(<lourenco_silva_2023>) introduce the concept of Team Size Reduction (TSR), which indicates if the average team size is shorter after the decomposition, by comparing the average number of authors per microservice to the total number of authors.
...
#cite_full(<mazlami_etal_2017>) make use of the TSR metric, as well as the Average Domain Redundancy (ADR) metric, which represents the amount of domain-specific duplication or redundancy between the microservices.
...
#cite_full(<carvalho_etal_2020>) propose a metric called reuse, which measures the reusability of a microservice.
...
#cite_full(<saidi_etal_2023>) use the intra-domain and inter-domain data dependency metrics to delineate microservice boundaries, based on the read and write access pattern of the operations.
In a similar fashion, #cite_full(<selmadji_etal_2020>) talk about data autonomy determined by the internal and external data access of a microservice candidate.

#cite_full(<kamimura_etal_2018>) introduce a metric called dedication score, which measures the relationships between services as a function of access frequency.
...
#cite_full(<ma_etal_2022>) use the Adjusted Rand Index (ARI) as clustering evaluation criterion.
...
#cite_full(<hao_etal_2023>) use the Matching Degree metric as quality indicator.
...
#cite_full(<hasan_etal_2023>) and #cite_full(<kalia_etal_2021>) use the Size metric to evaluate the quality of the microservice decomposition.
