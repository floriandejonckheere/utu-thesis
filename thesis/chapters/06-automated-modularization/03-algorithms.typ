...
Examples of publications using K-Means clustering to identify microservice candidates are #slr_cite_full(<saidi_etal_2023>), #slr_cite_full(<wu_zhang_2022>), #slr_cite_full(<romani_etal_2022>), and #slr_cite_full(<hao_etal_2023>).

#slr_cite_full(<al_debagy_martinek_2020>) use Affinity Propagation @frey_dueck_2007 to cluster vector representations of operation names in a software system.
...
#slr_cite_full(<lourenco_silva_2023>) uses similarity between domain entities accesses and development history of source code files as a guiding measure for the clustering algorithm, while #slr_cite_full(<zaragoza_etal_2022>) uses structural and data cohesion of microservices.
#slr_cite_full(<daoud_etal_2020>) extend the hierarchical agglomerative clustering (HAC) algorithm @murtagh_legendre_2014 with a collaborative approach, where the clustering is performed by multiple homogenous clustering nodes, each responsible for a subset of the data.

#slr_cite_full(<selmadji_etal_2020>) propose two possible algorithms for microservice identification: a hierarchical clustering algorithm, and a clustering algorithm based on gravity centers.

#slr_cite_full(<sellami_etal_2022>) use the Density-Based Spatial Clustering of Applications with Noise (DBSCAN) algorithm @ester_etal_1996 to identify microservices.
...
Examples of publications using Non-Dominated Sorting Algorithm II (NGSA-II) as multi-objective optimization algorithm to identify microservice candidates are #slr_cite_full(<zhou_xiong_2022>), #slr_cite_full(<kinoshita_kanuka_2022>), #slr_cite_full(<zhang_etal_2020>), #slr_cite_full(<jin_etal_2021>), and #slr_cite_full(<li_etal_2023>).
#slr_cite_full(<carvalho_etal_2020>) use the next generation of NSGA, NSGA-III, in order to find a solution for the problem.

#slr_cite_full(<amiri_2018>) rely on a genetic algorithm using Turbo-MQ @mitchell_etal_2001 as fitness function.
...
For example, #slr_cite_full(<mazlami_etal_2017>) and #slr_cite_full(<yang_etal_2022>) use Kruskal's algorithm @kleinberg_tardos_2006 to partition the graph into connected clusters.
Kruskal's algorithm is a greedy algorithm that finds the minimum spanning forest for an undirected weighted graph.

#slr_cite_full(<filippone_etal_2023>) apply the Louvain community detection algorithm @blondel_etal_2008 to obtain the granularity of the microservices, and high-cohesive communities of nodes.
...
#slr_cite_full(<hasan_etal_2023>) use the Leiden algorithm @traag_etal_2019, an improvement of the Louvain method that uses a refinement step to improve the quality of the communities.

#slr_cite_full(<cao_zhang_2022>) use both the Leiden algorithm and the hierarchical clustering algorithm to identify microservice candidates.
...
#slr_cite_full(<nitin_etal_2022>) use Context sensitive Label Propagation (CARGO), an algorithm built on the principles of the Label Propagation algorithm @zhu_ghahramani_2003.
...
For example, the authors of #slr_cite_full(<quattrocchi_etal_2024>) incorporated a Mixed Integer Linear Programming (MILP) solver in their solution.
...
#slr_cite_full(<filippone_etal_2021>) use a linear optimization algorithm to solve a combinatorial optimization problem.

The approach taken by #slr_cite_full(<kamimura_etal_2018>) is to use a custom clustering algorithm named SArF @kobayashi_etal_2012, that aims at identifying software subsystems without the need for human intervention.
#slr_cite_full(<escobar_etal_2016>) also use a custom clustering algorithm, detecting optimal microservices based on a meta-model of the class hierarchy.

#slr_cite_full(<agarwal_etal_2021>) propose an algorithm based on seed expansion.
...
#slr_cite_full(<eyitemi_reiff_marganiec_2020>) use a rule-based approach to microservice candidate identification.
